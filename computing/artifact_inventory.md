...
| **Docker image** | training: `…/rts-train:v2`; inference: `…/rts-infer:v1` (**rebuilt+pushed 2026-07-07**, `rts.git_sha=d56e7ef`, digest `sha256:b016eedb…`) | PDG Artifact Registry / us-west1 | — | `rts-infer:v1` is self-contained — current code incl. `inference/` (**async prob-COG writes + cached GCS client — the 12× throughput fix**), cv2 patch baked, MLflow 2.22.5 (pin `<3.0`); **no runtime sed/mount/pip**. Built locally + pushed via ADC (Cloud Build blocked, §8). `:v2` kept for training reproducibility. |
